...
colours = np.array(sorted([[idx, col] for col, com in enumerate(communities)
                    for idx in com], key=lambda x: x[0]))[:, 1]
...
_ = draw_kkl(G, None, colours, pos=pos, title="Pre-training", cmap='gist_rainbow',
             edge_color='gray')


train_nodes = np.array([0, 1, 8])
test_nodes = np.array([i for i in range(labels.shape[0])
                       if i not in train_nodes])
